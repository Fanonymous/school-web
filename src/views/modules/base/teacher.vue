	  ...
	  <el-card class="blue-shadow">
		  <el-form :inline="true">
		    ...
		    <el-form-item class="item-right">
		      <el-input v-model="keywords" placeholder="请输入老师姓名查询" clearable @input="fetchData"></el-input>
		    </el-form-item>
		    <!--<el-form-item class="item-right">
		  ...
		  <el-table :data="list" v-loading="listLoading" border fit highlight-current-row>
		    ...
		    <el-table-column align="center" label="教职工状态" >
		      <template slot-scope="scope">
		        <div v-if="scope.row.state == 0">在职</div>
		        <div v-else-if="scope.row.state == 1">离职</div>
		        <div v-else></div>
		      </template>
		    </el-table-column>
		  </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      fetchData() {
        ...
        let qtem = {limit:this.pagesize,order:"",page:this.currentPage,sidx:"",teacherName:this.keywords};
        if(this.schoolId !=0){
          qtem['schoolId']=this.schoolId;
        }
        var that = this
        getCommHttpData({url:'basedatacenter/teacher/queryByCondition',method:'post',contentType:2,data:qtem},function(data){
          if(data.code == 0){
