...
<!-- 		<el-select class="filter-item" v-model="schoolType" clearable @change="getGradeList" v-if="schoolTypeShow" placeholder="请选择学段">
    ...
    学段：
    <el-radio-group size="medium" v-model="schoolType" clearable @change="iniChartsData" v-if="schoolTypeShow">
      <el-radio-button label="全部" value="全部"></el-radio-button>
      <el-radio-button v-for="item in schoolTypeList" :key="item.name" :label="item.name" :value="item.name"></el-radio-button>
    </el-radio-group>
	...
	export default {
		...
		mounted() {
			...
			if(this.schoolTypeShow) {
				...
				let that = this
				getCommHttpData({
					url: 'equipAnalysis/getSchoolType',
					method: 'get',
					contentType: 1,
					data: {}
				}, function(data) {
