<template>
	<div style="display: inline-block;margin-bottom: 10px;">
		...
		<el-select class="filter-item" v-model="schoolType" clearable @change="getSchoolList" v-if="schoolTypeShow" placeholder="请选择学段">
			<el-option v-for="(item,index) in schoolTypeList" :key="item.id" :label="item.name" :value="index">
			</el-option>
		</el-select>
    ...
    <el-input v-model="keywords" placeholder="请输入学校名称搜索" clearable style="width: 180px;"></el-input>
	</div>
</template>
...
<script>
    ...
    watch: {
      keywords (val) {
        ...
        filterParams['schoolType'] = this.schoolTypeList[this.schoolType].name;
        filterParams['schoolSection'] = this.schoolTypeList[this.schoolType].id;
				let that = this
				getCommHttpData({
					url: 'multimemediaEquip/commFilter',
					method: 'post',
      ...
      getSchoolList(){
        let tempParams={}
        tempParams['commType'] = 4;
        if(this.schoolType.toString.length != ''){
          tempParams['schoolType'] = this.schoolTypeList[this.schoolType].name;
          //tempParams['schoolSection'] = this.schoolTypeList[this.schoolType].id;
        }
        let that = this
        getCommHttpData({
        	url: 'multimemediaEquip/commFilter',
          ...
          that.iniChartsData()
			...
			iniChartsData() {
				...
				if(this.schoolTypeShow) {
					if(this.schoolType != '') {
            jsonTemp['schoolType'] = this.schoolTypeList[this.schoolType].name;
            jsonTemp['schoolSection'] = this.schoolTypeList[this.schoolType].id;

					} else {
						jsonTemp['schoolType'] = ''
            jsonTemp['schoolSection'] = ''
					}
				} else {
					jsonTemp['schoolType'] = ''
          jsonTemp['schoolSection'] = ''
        ...
        jsonTemp['keywords'] = this.keywords ? this.keywords : null;
