<template>
	<div>
		<el-form-item :label="label" v-model="picSrc" :rules="{ required: fileRequired, message: '文件不能为空', trigger: 'change' }">
			<el-row :gutter="0">
				<!--<el-col :span="8">-->
					<template v-if="uploadPng == true">
						<el-col :span="8">
							<!-- 上传图片 -->
							<el-upload ref="upload" v-if="uploadPng == true" class="avatar-uploader" :action="apiUrl+'sys/oss/'+uploadUrl" :show-file-list="false" 
								:on-success="handleFileSuccess" :before-upload="deforeUpload" :data="uploadFile">
					...
					<template v-else>
						<!-- 上传文件 limit > 1 -->
						<el-col :span="16" v-if="limit >1">
							<el-upload ref="upload" class="upload-demo" :action="apiUrl+'sys/oss/'+uploadUrl" :on-preview="handleFilePreview" 
								:on-remove="handleFileRemove" :before-upload="deforeUpload" :on-success="handleFileSuccess" multiple :data="uploadFile" :limit="limit" 
								:on-exceed="handleFileExceed" list-type="picture-card" :file-list="picList">
						...
						<el-col :span="8" v-else>
							<el-upload ref="upload" class="upload-demo" :multiple=false :action="apiUrl+'sys/oss/'+uploadUrl" :on-preview="handleFilePreview" 
