<template>
   <div v-clickoutsides:exactAreaClassName="closeTree" style="display:inline-block;position: relative;margin-bottom: 10px;">
   	<div>
   	...
   	<div v-show="isShowSelect" class="search-tree exactAreaClassName">
   		...
   		<el-tree style="min-width: 200px;" :default-checked-keys="defaultChecked" :check-strictly="true" node-key="deptId" @node-click="nodeClick" show-checkbox default-expand-all empty-text="暂无数据" :props="defaultProps" :highlight-current=true
  ...
  watch: {
    ...
    getDeptList(){
    	...
    	getCommHttpData({
    		...
    		method: 'get',
    		contentType: 1,
    		data: {deptIds:this.deptId,deptType:this.userType}
    	}, function(data) {
    		that.selfDeptList = data.page
        that.deptIdSelect = data.page[0].children[0].deptId
        that.defaultChecked = [data.page[0].children[0].deptId]
        that.deptNameSelect = data.page[0].children[0].name
        that.$refs.selectTree.setCheckedNodes(data.page[0].children[0])
        that.$emit('refreshDataList',that.deptIdSelect)
    	})
    },
    selectClassfy(data,checked, node) {
      ...
      this.$emit('refreshDataList',this.deptIdSelect)
    },
