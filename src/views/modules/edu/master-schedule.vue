	...
	<div class="edu-class">
				...
				<template v-else>
					<div class="no-course-list" :style="'height: '+appHeight+'px'">
						<div><img src="~@/assets/img/nodata2.png" /></div>
							<div>暂未录入课表，请点击录入课表。
								<el-button type="primary" 
									@click="$router.push({path:'/edu-add-m-course',query: {termId:termId,schoolYearId:schoolYearId, schoolId : schoolId,yearTermId : yearTermId,gradeId :gradeId,yearTermName:yearTermName,gradeName:gradeName}})">录入课表</el-button>
							</div>
					</div>
				</template>
			</section>
		...
		<el-dialog title="" :visible.sync="dialogFormVisible" width="290px">
			<div class="qrcode-box">
				<div class="qrcode-title margin-bottom-20">{{courseName ? courseName : yearTermName + gradeName + className+'课程表'}}课程表</div>
	...
	export default {
		...
		methods: {
				...
				getCommHttpData({
					...
					if(data.code == 0) {
						...
						if(data.list && data.list.length > 0){
							...
							that.gradeId = params.gradeId ? params.gradeId : data.list[0].gradeId
							that.gradeName = params.gradeName ? params.gradeName : data.list[0].gradeName
						}else{
							that.gradeId = ''
							that.gradeName = ''	
						}
			...
			getClassData() {
				var that = this
				getCommHttpData({
					url: 'eduManageClass/queryGradeClass',
					method: 'get',
					contentType: 1,
					data: {
