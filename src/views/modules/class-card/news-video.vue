<template>
    <div class="new-video-wrapper" v-loading="allLoading">
        <div class="news-letter-wrapper-box">
            <div class="news-title" v-show="isPreview == 1">
                <el-input v-model="title" maxlength="15" placeholder="点击此处添加标题" :class="{'white-color-blank' : formModel.backgroundColor == 2, 'blank-color-white' : formModel.backgroundColor == 1}"></el-input>
            </div>
            <div class="new-content" v-show="isPreview == 1">
                <el-upload class="avatar-uploader" :action="apiUrl+'/sys/oss/viedoUpload'" :show-file-list="false" :file-list="imagesList"
                    :on-success="handleFileSuccess" :before-upload="deforeUpload" :on-error="handFileError">
                    <img :src="baseIp + fileList[0].response.viedoImage" alt="视频已上传" v-if="imagesList.length || fileList.length" class="avatar">
                    <i v-else class="el-icon-plus avatar-uploader-icon">
                        <span>选择视频</span>
                    </i>
                    <div slot="tip" class="tip-formate" v-if="!imagesList.length && !fileList.length">提示：只能上传mov、rmvb、rm、flv、mp4、3gp文件，且不超过1G</div>

                    <div class="prograss-upload" v-if="isShowPercent">
                        <el-progress type="circle" :percentage="percentageValShow" class="prograss-item"></el-progress>
                    </div>
                </el-upload>
           ...
           <div class="preview-wrapper" v-show="isPreview == 2 || isPreview == 3">
                <div class="preview-box">
                    <div :class="{'white-color-blanks' : formModel.backgroundColor == 2, 'blank-color-whites' : formModel.backgroundColor == 1}">
                    ...
                    <div class="preview-content">
                        <video :src="imagesList[0].url" v-if="fileList.length && imagesList.length" controls="controls" autoplay></video>
                        <video :src="baseIp + fileList[0].response.url" v-if="fileList.length && !imagesList.length" controls="controls" autoplay></video>
                    </div>
                </div>
...
export default {
    ...
    computed : {
        ...
        percentageValShow() {
            return Math.ceil(this.percentageVal)
        }
    },
    mounted() {
    ...
    methods : {
        ...
        deforeUpload(file) {
            ...
            let isBig = file.size / 1024 / 1024 < 1024, isFormate = true
            ...
            if(fileExt != ".MOV" && fileExt != ".RMVB" && fileExt != ".RM" && fileExt != ".FLV" && fileExt != ".MP4" && fileExt != ".3GP") {
            ...
            if (isBig && isFormate) {
                let timer = setInterval(_ => {
                    let n = 50 / (file.size / 1024 / 1024)
                    if (this.percentageVal < 50) {
                        this.percentageVal += n
                    }else {
                        this.percentageVal < 90 && (this.percentageVal += 1)
                        this.percentageVal < 98 && (this.percentageVal += 0.1)
                        this.percentageVal > 98 && (this.percentageVal += 0.000000000000001)
                    }
                }, 100)
        ...
        handFileError(err, file, fileList) {
            this.$message.error('上传失败!')
            this.isShowPercent = false
        },
        handClickPreview() {
        ...
        handCancel() {
            if (this.fileList.length && !this.broadcastId) {
