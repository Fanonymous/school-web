<template>
    <div class="new-image-wrapper" v-loading="loading">
        <div class="news-letter-wrapper-box">
            ...
            <div class="new-content" v-show="isPreview == 1">
                <el-upload list-type="picture-card" :action="apiUrl+'/sys/oss/picUpload'" ref="uploadpic" :limit="12" :file-list="imagesList" :on-remove="handFileRemove"
                            :on-exceed="handleExceed" :on-success="handleFileSuccess" :before-upload="deforeUpload" :data="imgName" style="padding:10px;"
                            multiple accept="image/jpeg,image/gif,image/png,image/jpg">
                    <i class="el-icon-plus">
                        <div class="chose-image-mess">选择图片</div>
                    </i>
                </el-upload>
            </div>
           ...
           <div class="preview-wrapper" v-show="isPreview == 2 || isPreview == 3">
                <div class="preview-box">
                    ...
                    <div class="preview-content">
                        <el-carousel indicator-position="outside" :interval="10000" :initial-index="0" height="500px" arrow="always" v-if="!imagesList.length && fileList.length">
                            <el-carousel-item v-for="(item, index) in fileList" :key="index">
                                <img :src="baseIp + item.response.url" class="image" alt="加载中..." style="max-height: 100%;width:800px;">
                            </el-carousel-item>
                        </el-carousel>
                        <el-carousel indicator-position="outside" :interval="10000" :initial-index="0" height="500px" v-if="imagesList.length && fileList.length">
                            <el-carousel-item v-for="(item, index) in imagesList" :key="index">
                                <img :src="item.url" class="image" alt="加载中..." style="max-height: 100%;width:800px;">
                            </el-carousel-item>
                        </el-carousel>
                    </div>
                </div>
    ...
    methods : {
        ...
        deforeUpload(file) {
            ...
            if(fileExt != ".PNG" && fileExt != ".GIF" && fileExt != ".JPG" && fileExt != ".JPEG" && fileExt != ".BMP") {
                ...
                let is = false
                if (this.imagesNameObj[file.name]) {
        ...
        handleFileSuccess(response, file, fileList) {
            if (response.code == 0) {
                ...
                this.imagesNameObj[file.name] = true
            }else {
        ...
        handFileRemove(file, fileList) {
            ...
            fileList.forEach(item => {
                this.imagesNameObj[item.name] = true
            })
            this.fileList = fileList
        },
