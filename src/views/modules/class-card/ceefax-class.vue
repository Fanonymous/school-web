		...
		<el-card class="blue-shadow">
			<el-row>
				<el-col :span="24">
					...
					<!--<el-select v-model="theType" placeholder="图文资讯类型" @change="getCeefax" style="margin-right: 3px;">
					...
					<el-input placeholder="请输入名称" :inline="true" style="width: 200px;" v-model="msg" @input="getCeefax"></el-input>
			...
			<div class="table" style="padding-top: 10px;">
				<el-table :data="tableList"  border >
					...
					<el-table-column prop="address" v-if="theType==2002" label="班级" align="center">
						<template slot-scope="scope">
					         <p>{{scope.row.gradeName}}{{scope.row.className}}</p>
					   </template>
					</el-table-column>
...
export default {
    ...
    methods:{
        handView(v, id) {
            if (v) {
                this.$router.push( { name : 'edit_publishs', query : { informationId : id } } )
            }else {
                this.$router.push( { name : 'view_news', query : { informationId : id, isView : '1' } } )
            }
        },

        handJump(mark) {
            this.$router.push({ name : 'publish_infors', query : { theType : this.theType, mark : mark } })
        },
        ...
        getCeefax(){
            ...
            if(this.theType==2002){
                if(this.gradeClassId != undefined && this.gradeClassId.toString().length >0){
