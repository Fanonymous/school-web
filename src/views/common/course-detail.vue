			...
			<template v-if="list.length > 0">
				...
				<table id="class-table" width="100%" cellspacing="0">
					<tbody>
						<tr>
							<th width="10%"></th>
							<th :width="(90/weekDay.length)+'%'" v-for="item in weekDay">
								{{item}}
							</th>
						</tr>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getCourseList() {
				let schoolId, yearTermId, gradeId, classId;
				if(this.$route.query.schoolId) {
					...
					getCommHttpData({
						url: 'eduManageCourse/classCourseList?schoolId=' + schoolId + '&yearTermId=' + yearTermId + '&gradeId=' + gradeId + '&classId=' + classId,
						method: 'get',
						contentType: 2
					}, function(data) {
